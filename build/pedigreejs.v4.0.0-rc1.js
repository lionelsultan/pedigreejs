var pedigreejs = (function (exports) {
	...
	function include_children(connected, p, dataset) {
	  ...
	  const getAllChildren = (dataset, person, sex) => {
	    return $.map(dataset, function (p, _i) {
	      return !('noparents' in p) && (p.mother === person.name || p.father === person.name) && (true) ? p : null;
	    });
	  };
	...
	**/
	...
	function clear_browser_store(opts) {
	  ...
	  for (let i = 0; i < store.length; i++) {
	    let key = store.key(i);
	    if (key && key.indexOf(prefix) === 0) items.push(key);
	  }
	...
	function clear(opts) {
	  if (opts) {
	    ...
	    Object.keys(dict_cache).forEach(function (key) {
	      if (key.indexOf(prefix) === 0) delete dict_cache[key];
	    });
	  } else {
	    try {
	      localStorage.clear();
	...
	function setposition(opts, x, y, zoom) {
	  if (has_browser_storage(opts)) {
	    ...
	    if (zoom !== null && zoom !== undefined) set_browser_store(opts, zoomName, zoom);else store.removeItem(zoomName);
	  } else {
	    ...
	    let zoomName = get_prefix(opts) + '_ZOOM';
	    if (zoom !== null && zoom !== undefined) dict_cache[zoomName] = zoom;else delete dict_cache[zoomName];
	  }
	}
	function getposition(opts) {
	  if (has_browser_storage(opts)) {
	    if (localStorage.getItem(get_prefix(opts) + '_X') === null && sessionStorage.getItem(get_prefix(opts) + '_X') === null) return [null, null];
	    ...
	    if (dict_cache[get_prefix(opts) + '_X'] === null || dict_cache[get_prefix(opts) + '_X'] === undefined) return [null, null];
	    let pos = [parseInt(dict_cache[get_prefix(opts) + '_X']), parseInt(dict_cache[get_prefix(opts) + '_Y'])];
	    if (dict_cache[get_prefix(opts) + '_ZOOM'] !== null && dict_cache[get_prefix(opts) + '_ZOOM'] !== undefined) pos.push(parseFloat(dict_cache[get_prefix(opts) + '_ZOOM']));
	    return pos;
	  }
	}
	...
	**/
	...
	function getNodeByName(nodes, name) {
	  ...
	  if (nodes && nodes.dataset) {
	    for (let j = 0; j < nodes.dataset.length; j++) {
	      if (name === nodes.dataset[j].name) return {
	        data: nodes.dataset[j]
	      };
	    }
	  }
	}
	...
	function flatten(root) {
	  ...
	  recurse(root);
	  if (root && root._dataset) flat.dataset = root._dataset;
	  return flat;
	}
	...
	function copy_dataset(dataset) {
	  ...
	  return dataset.map(function (person) {
	    ...
	    for (let key in person) {
	      if (!Object.prototype.hasOwnProperty.call(person, key) || disallowed.has(key)) continue;
	...
	**/
	 ...
	 */
	function addpartner(opts, dataset, name, config) {
	  ...
	  let motherExists = tree_node.data.mother && getIdxByName(dataset, tree_node.data.mother) !== -1;
	  let fatherExists = tree_node.data.father && getIdxByName(dataset, tree_node.data.father) !== -1;
	  if (tree_node.data.top_level || !tree_node.data.mother && !tree_node.data.father) {
	    ...
	    if (motherExists) partner.mother = tree_node.data.mother;
	    if (fatherExists) partner.father = tree_node.data.father;
	    // If parents are missing in the dataset, fall back to top_level to keep the partner visible
	    if (!partner.mother && !partner.father) partner.top_level = true;
	  }
	  partner.noparents = true;
	...
	**/
	...
	function delete_node_dataset(dataset, node, opts, onDone) {
	  ...
	  try {
	    ...
	    if (opts && opts.dataset) baselineDisconnected = unconnected(opts.dataset);
	  } catch (err) {
	  ...
	  if (uc.length > 0) {
	    if (!hadDisconnectedBefore) {
	      ...
	      messages("Warning", "Deleting this will split the pedigree. Continue?", confirmCallback, opts, dataset);
	      return;
	    }
	  }
	  if (onDone) {
	...
	function addWidgets(opts, node) {
	  ...
	  for (let key in widgets) {
	    let widget = node.filter(function (d) {
	      ...
	      return (d.data.hidden && !opts.DEBUG ? false : true) && !((d.data.mother === undefined || d.data.noparents) && key === 'addsibling') && !(d.data.parent_node === undefined && key === 'addchild') && !(d.data.noparents === undefined && d.data.top_level === undefined && key === 'addparents');
	  ...
	  node.selectAll(".addsibling, .addchild").on("mouseover", function () {
	  ...
	  // FIX: Use node.selectAll to scope selection to widgets created in this pedigree instance
	  node.selectAll(".addchild, .addpartner, .addparents, .delete, .settings").on("click", function (e) {
	    e.stopPropagation();
	...
	**/
	...
	function build(options) {
	  ...
	  ped.selectAll(".link").data(root.links(nodes.descendants())).enter().filter(function (d) {
	    ...
	    if (d.source.data.mother && d.source.data.father) {
	...
	function check_ptr_links(opts, ptrLinkNodes) {
	  ...
	  for (let a = 0; a < ptrLinkNodes.length; a++) {
	    ...
	    if (clash) {
	      ...
	      if (opts.DEBUG) console.log("CLASH :: " + ptrLinkNodes[a].mother.data.name + " " + ptrLinkNodes[a].father.data.name, clash);
	    }
	  }
	  return clashes;
	}
	function check_ptr_link_clashes(opts, anode) {
	  ...
	  if ('name' in anode) {
	    ...
	    if (!anode || !('mother' in anode.data)) return null;
	    ...
	    mother = getNodeByName(flattenNodes, motherName);
	    father = getNodeByName(flattenNodes, fatherName);
	  } else {
	    mother = anode.mother;
	    father = anode.father;
	  }
	  ...
	  if (mother.data === undefined || father.data === undefined) {
	    mother = getNodeByName(flattenNodes, mother.name || mother.data?.name);
	    father = getNodeByName(flattenNodes, father.name || father.data?.name);
	  }
	  if (!mother || !father || mother.x === undefined || father.x === undefined) return null;
